<x-layouts.admin title="Artikel">
    <div class="rounded-card border border-line bg-surface p-6 shadow-sm">
        ...
        <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <x-input name="title" label="Judul Artikel" placeholder="Tulis judul artikel" class="rounded-full" value="{{ old('title') }}" />

            <x-textarea name="content" label="Isi Artikel" :rows="12" placeholder="Tulis isi artikel di sini...">{{ old('content') }}</x-textarea>
